...
class HandleImgUtils {
    public static String opencv_work_dir = "/sdcard/opencv/";
    public static String opencv_work_dir2 = "/sdcard/opencv/p/";
...
//        img = cv2.imread(‘D:\\test\\contour.jpg’)
        ...
        Imgproc.threshold(cannyMat,binaryMat, 127,255,Imgproc.THRESH_BINARY);
...
//        cv2.drawContours(img,contours,-1,(0,0,255),3)
        ...
        // 寻找轮廓
        Imgproc.findContours(binaryMat, contours, hierarchy, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_NONE,
                new Point(0, 0));

        Imgproc.drawContours(binaryMat, contours, -1, new Scalar(0, 255, 0));
        saveImg(binaryMat, opencv_work_dir + "binary.jpg");
        // 找出匹配到的最大轮廓
        double area = Imgproc.boundingRect(contours.get(0)).area();
        int index = 0;
        ...
        MatOfPoint2f maxMp = new MatOfPoint2f(contours.get(index).toArray());

        // 原始曲线与近似曲线之间的最大距离设置为0.01，true表示是闭合的曲线
        Imgproc.approxPolyDP(maxMp, approxCurve, 0.01, true);
    ...
    /**
     ...
     @param points          逼近的点集
     @param referencePoints 四个参照点集(通过寻找最大轮廓，进行minAreaRect得到四个点[左上，右上，右下，左下])
     */
    ...
    /**
     获取四个顶点的参照点<br/>
     返回Point数组[左上，右上，右下，左下] <br/>
     思路： 我们可以把四个点分成两部分，左部分，右部分<br/>
     左部分：高的为左上，低的为左下(高低是以人的视觉)<br/>
     右部分同理 首先我们找到最左和最右的位置，以它们的两个中间为分界点，
     靠左的划分到左部分，靠右的划分到右部分 如果一个区域有三个或更多，
     哪个比较靠近分界线，划分到少的那个区域
     @param cannyMat
     @return
     */
    ...
    public static double caculatePointToPoint(Point p1, Point p2) {
        double tempDistance = Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2);
        return tempDistance;
    }
    /**
     具体的寻找四个顶点的坐标
     @param map 四个点集域 即左上，右上，右下，左下
     @return
     */
    public static Point[] specificFindFourPoint(Map<String, List> map) {
        ...
        for (i = 0; i < px1.size(); i++) {
            for (j = 0; j < py2.size(); j++) {
                ...
                Point point1 = px1.get(i);
                Point point12 = py2.get(i);

                tempDistance = caculatePointToPoint(point1, point12);
//                tempDistance = Math.pow(point1.x - py2.get(j).x, 2) + Math.pow(point1.y - py2.get(j).y, 2);
        ...
        for (i = 0; i < px2.size(); i++) {
            for (j = 0; j < py1.size(); j++) {
                Point point1 = px2.get(i);
                Point point12 = py1.get(i);
                tempDistance = caculatePointToPoint(point1, point12);
//                tempDistance = Math.pow(px2.get(i).x - py1.get(j).x, 2) + Math.pow(px2.get(i).y - py1.get(j).y, 2);
    ...
    public static Point[] findFourPoint2(Mat src,Mat cannyMat) {
        ...
        for (int i = 0; i < points.length; i++) {
            setPixel(approxPolyMat, (int) points[i].y, (int) points[i].x, sampleColor);
        }

        saveImg(approxPolyMat, opencv_work_dir2+"x11-approxPolyMat.jpg");
        ...
        for (int i = 0; i < px1.size(); i++) {
            setPixel(areaMat, (int) px1.get(i).y, (int) px1.get(i).x, sampleColor);
        }

        for (int i = 0; i < px2.size(); i++) {
            setPixel(areaMat, (int) px2.get(i).y, (int) px2.get(i).x, sampleColor);
        }

        for (int i = 0; i < py1.size(); i++) {
            setPixel(areaMat, (int) py1.get(i).y, (int) py1.get(i).x, sampleColor);
        }

        for (int i = 0; i < py2.size(); i++) {
            setPixel(areaMat, (int) py2.get(i).y, (int) py2.get(i).x, sampleColor);
        }

        saveImg(areaMat, opencv_work_dir2+"x11-pointsDivideArea.jpg");
    ...
    public static Mat warpPerspective2(Mat src) {
        Mat finalmat = new Mat();
        return finalmat;
    }
    /**
     透视变换，矫正图像 思路：
     <dl><dt>寻找图像的四个顶点的坐标(重要) </dt>
     <dd>1、canny描边</dd>
     <dd>2、寻找最大轮廓</dd>
     <dd> 3、对最大轮廓点集合逼近，得到轮廓的大致点集合 </dd>
     <dd>4、把轮廓的点集合划分到四个区域中，即左上，右上，左下，右下  </dd>
     <dd>  5、根据矩形中，对角线最长，找到矩形的四个顶点坐标 </dd>
     <dd>  6、根据输入和输出点获得图像透视变换的矩阵 </dd>
     <dd>  7、透视变换 </dd>
     </dl>
     @param src
     */
    public static Mat warpPerspective(Mat src) {
        // 灰度话
     Mat graySrc = HandleImgUtils.gray(src);
        // Canny
        Mat cannyMat = HandleImgUtils.canny(graySrc);
        // 找到四个点
        Point[] points = HandleImgUtils.findFourPoint2(graySrc,cannyMat);
    ...
    public static void cutRect(Mat correctMat, Mat nativeCorrectMat) {
        ...
        HandleImgUtils.saveImg(t, opencv_work_dir + "cutRect.jpg");
    }
    ...
    public static void correct(Mat src) {
        ...
        HandleImgUtils.saveImg(src, opencv_work_dir + "srcImg.jpg");

        HandleImgUtils.saveImg(CorrectImg, opencv_work_dir + "correct.jpg");
    }
